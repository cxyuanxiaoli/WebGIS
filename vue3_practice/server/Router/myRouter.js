...
router.use((req,res,next)=>{
  fs.appendFile('./data/log.txt', new Date().toDateString() + ' ' + new Date().toTimeString() +
    ' ' + req.url + ',' + req.method + '\n', 'utf-8', (err, data) => {
      if (err) {
        console.log(err)  
      }
...
router.post('/login', (req, res) => {
  ...
  console.log(body)
  db.query(`select count(*) from usertable where username='${body.username}' and 
      password='${body.password}'`, (err, result) => {
    if (err) {
      console.log(err.message)
      res.json({ success: false, msg: err.message })
      return
...
router.get('/data/geojson',(req,res)=>{
  res.sendFile('D:/文件夹/WebGIS/webProj/server/data/山东省.geoJson')
})
